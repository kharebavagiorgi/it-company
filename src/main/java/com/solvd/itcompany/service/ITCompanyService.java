...
public class ITCompanyService {
    ...
    public void assignTask(Manageable manager, Task task) {
        ...
        LOGGER.info("Assigning task via a Manageable interface.");
        manager.assignTask(task);
        if (task.isCompleted()) {
            LOGGER.debug("Task '{}' has been marked as completed.", task.getTaskName());
        }
    }

    public void generateProjectReport(Reportable project) {
       LOGGER.info("Generating report via a Reportable interface.");
        project.generateReport();
    }

    public BigDecimal getPayableSalary(Payable payable) {
        LOGGER.info("Calculating salary via a Payable interface.");
        return payable.calculateSalary();
    }

    public void performAudit(Auditable auditable) {
        LOGGER.info("Performing audit via an Auditable interface.");
    ...
    public void demonstrateStreams(ITCompany company) {
        ...
        List<Developer> midLevelDevelopers = allEmployees.stream()
                ...
                .collect(Collectors.toList());
        LOGGER.debug("2. Mid-Level Developers ({}): {}", midLevelDevelopers.size(), midLevelDevelopers.stream().map(Employee::getName).collect(Collectors.joining(", ")));

        String projectNames = company.getProjects().stream()
                .map(p -> p.getProjectName().toUpperCase())
                .collect(Collectors.joining(" | "));
        LOGGER.info("3. All Project Names: " + projectNames);

        long highPriorityCount = company.getProjects().stream()
                .filter(p -> p.getPriority() == Priority.HIGH)
                .count();
        LOGGER.info("4. High Priority Projects Count: {}", highPriorityCount);
        ...
        double avgYoE = allEmployees.stream()
                ...
                .average()
                .orElse(0.0);
        LOGGER.info("5. Average Developer Years of Experience: %.2f%n", avgYoE);

        Employee highestPaid = allEmployees.stream()
                .max((e1, e2) -> e1.getSalary().compareTo(e2.getSalary()))
                .orElse(null);
        LOGGER.info("6. Highest Paid Employee: {}", highestPaid != null ? highestPaid.getName() + " ($" + highestPaid.getSalary() + ")" : "N/A");

        Map<Boolean, List<Employee>> rolePartition = allEmployees.stream()
                .collect(Collectors.partitioningBy(e -> e instanceof Developer));

        LOGGER.info("7. Partition: Developers ({}) vs Other Roles ({})", rolePartition.get(true).size(), rolePartition.get(false).size());
    }
    ...
    public void processEmployeesWithLambdas(ITCompany company) {
        ...
        Consumer<Project> advanceStatus = p -> {
            ...
            LOGGER.debug("Project '%s' moved from %s to %s.%n", p.getProjectName(), oldStatus, p.getStatus());
        };
        ...
        BiConsumer<Project, Double> updateExpenses = (p, exp) -> {
            ...
            LOGGER.debug("Project '%s' added $%,.2f in expenses.%n", p.getProjectName(), exp);
        };
